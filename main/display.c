...
/**
 * @brief Draws text at coordinates
 * @param x Top left x
 * @param y Top left y
...
void draw_text(esp_lcd_panel_handle_t panel_handle, uint16_t x, uint16_t y, const char *text, uint16_t color, uint16_t bg_color) {
    ...
    while (*text) {
        ...
        const uint8_t *char_bitmap = font8x8[*text - 32];
        uint16_t *buf = heap_caps_malloc(8 * 8 * sizeof(uint16_t), MALLOC_CAP_DMA);
        if (!buf) return;
        
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                if (char_bitmap[row] & (1 << (7 - col))) {
                    buf[row * 8 + col] = swapped_color;
                } else {
                    buf[row * 8 + col] = swapped_bg;
                }
            }
        }
        
        esp_lcd_panel_draw_bitmap(panel_handle, x, y, x + 8, y + 8, buf);
        free(buf);
        x += 8;
        text++;
    }
